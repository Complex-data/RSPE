...
class Model:
    ...
    def _joint_representation(self, q1_embed, q2_embed, q1_len, q2_len, q1_max,
        ...
        if self.args.implicit == 1:
            q1_output = tf.concat([q1_output, self.user_batch,self.user_emb_batch], 1)
            q2_output = tf.concat([q2_output, self.item_batch, self.item_emb_batch], 1)

        
        self.u1_size = q1_output
        self.u2_size = self.user_batch
        self.u3_size = self.user_emb_batch
        ...
        with self.graph.as_default():
            ...
            with tf.variable_scope('embedding_layer'):
                ...
                if self.args.implicit == 1:
                    ...
                    self.user_emb_batch = tf.nn.embedding_lookup(self.user_emb,
                                                             self.user_id)

                    self.item_emb_batch = tf.nn.embedding_lookup(self.item_emb,
                                                             self.item_id)
                # 添加category
                if self.args.category == 1:
